...
And, of course, the ln-posterior gradients with respect to the parameters $\theta$ must be multiplied by the jacobian of this transformation
\begin{eqnarray}
  \theta_i & = & g(z_i) \\
         & = & \theta_{min} + (\theta_{max} - \theta_{min}) \, \frac{1}{1 + e^{-z_i}} \\
  J_{t,i} & = & \frac{\partial \theta_i}{\partial z_i} \\
           &  = &  R \, g(z_i) \, (1 - g(z_i)) \\
...
\begin{eqnarray}
  ln|\det{J_t}|& = & ln | \product_i \frac{\partial \theta_i}{\partial z_i} | \\
                 & = & \\
\end{eqnarray}
Now the gradients of the ln-posterior become somewhat more complicated, since this Jacobian term also ahs to be differntiated.
Fortunately the logistic function we are using has easy second derivatives.
\begin{eqnarray}
 \frac{\partial \det{J_t}}{\partial z_i} & = & \frac{\det{J_t}}{J_{t,i}} \frac{\partial^2\theta_i}{\partial^2 z_i}\\
                 & = & \det{J_t} \, (1 - 2 \, g(z_i))
\end{eqnarray}
