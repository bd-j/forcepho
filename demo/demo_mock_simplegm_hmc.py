...
if __name__ == "__main__":
    ...
    upper = np.array([60., 20., 20., 1.0, np.pi/2.])
    lower = np.array([10., 5., 5., 0.0, -np.pi/2])
    ...
    if True:
        ...
        pos, prob, eps = sampler.sample(p0*1.2, model, iterations=iterations,
                                        epsilon=eps/10., length=length, store_trajectories=True)
        ...
        for i in range(ndim):
            ...
            taxes[i, 1].hist(sampler.trajectories[:, :, i].flatten(), alpha=0.5, bins=30)
