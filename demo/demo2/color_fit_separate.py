...
if __name__ == "__main__":
    ...
    parser.add_argument("--outbase", type=str, default="./output/separate_v1")
    # sampling
    ...
    for i, band in enumerate(bands):
        taskID = i + 1
        logger.info(f"Working on {band} band, with taskID={taskID}")

        # --- build the scene server ---
        cat = fits.getdata(config.image_names[0], -1)
        sceneDB = LinkedSuperScene(sourcecat=cat, bands=[band],
        ...
        # --- load the image data ---
        patcher = Patcher(fitsfiles=[config.image_names[i]],
        ...
        outroot = os.path.join(config.patch_dir, f"patch_{band}")
