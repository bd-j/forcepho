...
\end{eqnarray}
where $R$ is the rotation matrix parameterized by an angle, $S$ is the scale matrix, and $\Sigma_m$ is the covariance matrix describing the $m$th Gaussian.
The exponential in pixel space has 6 parameters $\phi_g$:
$A = (a_m \, a_\ell, \, \det{F}){2\pi}$,
$\mu_{m,\ell} = x_{m, \ell}, y_{m, \ell}$,
$F_{xx}$,
$F_{yy}$,
and $F_{xy}=F_{yx}$

There are 7 galaxy source parameter $theta_s$s:
$F$ the total flux,
$\alpha, \delta$ the celestial coordinates,
$n, r_h$ the profile parameters,
