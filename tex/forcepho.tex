...
Following \citet{hogg_mog}, but we: 
  ...
  2) fix the radii of the gaussians (in pixel space), including one at $r=0$;
...
where here $\delta_{i,j}$ is the Kroneker-$\delta$ and $r_m$ is the dispersion  of the $m$th  Gaussian (in pixels).
We also renormalize so that 
\begin{eqnarray}
...
The scalar $d$ is a dilation factor.
...
\begin{eqnarray}
I_{m, \ell}(\mu_p) & = & \, \frac{ \psi \, a_m \, a_\ell \, \sqrt{\det{F}}}{2\pi}  \, \exp(-0.5 \, \transpose{(\mu_p -\mu_{m,\ell})} \, F \, (\mu_p -\mu_{m,\ell})) \\
F & = & (\Sigma'_m + \Sigma_\ell)^{-1} \nonumber \\
\mu_{m,\ell} & = & \mu_m + \mu_\ell \nonumber \\
\Sigma'_m & = & \transpose{(R \, S)} \, \Sigma_m \, R\, S  \nonumber 
\end{eqnarray}
where $R$ is the rotation matrix parameterized by a position angle $\varphi$, $S$ is the scale matrix parameterized by the axis ratio $q$, and $\Sigma_m$ is the covariance matrix describing the $m$th Gaussian in the untransformed (circularly symmetric) space.
...
the Sersic index $n$, 
the half-light radius in  celestial units (e.g. degrees or arcseconds) $\rho_h$,
...
Need to add in the astrometric distortion affine transformtion to go from sky coordinates to pixel coordinates.
